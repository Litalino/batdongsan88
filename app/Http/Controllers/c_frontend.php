...
class c_frontend extends Controller
{
    ...
    public function dangky(Request $Request)
    {
        ...
        Mail::send('email_feedback', array(
            'name'=>$name,
            'phone'=>$phone,
            'email'=>$email,
            'link'=>$link,
            'date'=>$date
            ), function($message) use ($mail, $namesite){
            $message->from($mail, $namesite);
            $message->to($mail, $namesite)->subject('Thông tin khách hàng');
        });
